  ...
  if (payload.type === 'block_actions') {
    ...
    switch (action.action_id) {
      ...
      case 'dismiss':
        ...
        break;
      case 'approve':
        await api.postLeave(payload, JSON.parse(action.value));
        break;
      case 'reject':
        await api.rejectLeave(payload, JSON.parse(action.value));
        break;
    }
 ...
 * Verify the signing secret before continuing.
 */
app.post('/slack/options', async (req, res) => {
  if (!signature.isVerified(req)) return res.status(400).send();
  const payload = JSON.parse(req.body.payload);
  ...
  switch (payload.view.callback_id) {
    case 'request_leave':
      ...
      let leave = {
        ...
        reason: values.reason.reason_id.value,
        channels: channels,
        channelString: channelString
      }
      return res.send(payloads.confirm_leave({
        leave
      }));
    case 'confirm_leave':
      await api.requestLeave(payload.user, JSON.parse(payload.view.private_metadata));
